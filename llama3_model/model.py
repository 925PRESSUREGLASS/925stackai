...
class Llama3QuoteModel(nn.Module):
    ...
    def ollama_chat(self, user_message: str, system_prompt: Optional[str] = None, model: str = "llama3", host: str = "http://localhost:11434", max_new_tokens: int = 128) -> str:
        """
        ...
        try:
            ...
            return f"[Ollama unavailable: {e}]\n" + self.chat(user_message, system_prompt, max_new_tokens)
    def __init__(self, model_name: str = "meta-llama/Llama-3-8B", d_model: int = 512, n_heads: int = 8,
                 n_layers: int = 6, vocab_size: int = 32000, max_seq_len: int = 256):
        super().__init__()
        try:
    ...
    def generate_text(self, prompt: str, max_new_tokens: int = 64) -> str:
        ...
        if self.tokenizer:
            ...
            output_text = " ".join(id_to_token.get(i, "") for i in generated_ids[len(tokens):])
        return output_text

    def chat(self, user_message: str, system_prompt: Optional[str] = None, max_new_tokens: int = 64) -> str:
        """
        Generate a conversational response to a user message.
        Uses Ollama Llama 3 if available, otherwise falls back to local model.
        """
        try:
            return self.ollama_chat(user_message, system_prompt, max_new_tokens=max_new_tokens)
        except Exception:
