...
system_prompt = st.text_area("System Prompt (optional)", "You are a helpful quoting assistant.")
user_message = st.text_area("Quote Description", "I need a quote for replacing 10 windows in a two-story house.")
max_tokens = st.slider("Max New Tokens", 32, 512, 128)

if st.button("Generate Quote"):
    with st.spinner("Generating quote..."):
        try:
            response = st.session_state.model.chat(user_message, system_prompt, max_new_tokens=max_tokens)
